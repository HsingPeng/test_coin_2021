...
def calculate_one():
    ...
    for candle_begin_time, row in source_df.iterrows():
        ...
        if last_low_price is None:
        ...
        elif last_high_price - row['close'] > diff_price:
            ...
            last_low_price = None
            diff_price = None
        elif last_high_price < row['close']:
            last_high_price = row['close']

        show_num += 1
        if show_num % 10000 == 0:
            print(candle_begin_time, row['close'], last_high_price, last_low_price, diff_price, num_ret, num_ret_last_time)
